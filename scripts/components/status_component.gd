...
func add_modifier(stat_name: StringName, params: Dictionary) -> void:
	...
	if not _active_status.has(behavior): _active_status[behavior] = {}
	if not _active_status[behavior].has(stat_name): _active_status[behavior][stat_name] = []
...
func get_total_modifier(stat_name: StringName) -> float:
	var total := 0.0
...
func has_taunt() -> bool:
	return _has_behavior(StatusEffect.Behavior.PROTECT, &"TAUNT")
